...
await send("Page.navigate", { url: `${BASE}/category/apparel` });
await sleep(6000);
const cat = await ev(`(() => {
...
await shot(`${OUT}/task72-category-apparel.png`);

// ---- 2. PDP with category link ----
await send("Page.navigate", { url: `${BASE}/product/carry-canvas-tote` });
await sleep(6000);
const pdp = await ev(`(() => {
...
await shot(`${OUT}/task72-pdp-category-link.png`);
...
await shot(`${OUT}/task72-branded-404.png`);
