// Task 59 live verify: support@hayaan.co surfaces — footer contact link,
// quote-page email line. Screenshots to download/.
...
const chrome = spawn(CHROME, [
  ...
  `--user-data-dir=/tmp/chrome-t59-${Date.now()}`,
...
// ---- 1. homepage footer: mailto link present? scroll + screenshot footer ----
await send("Page.navigate", { url: `${BASE}/` });
await sleep(8000);
console.log("footer mailto:", await ev(`(() => {
  const a = [...document.querySelectorAll('footer a[href^="mailto:"], a[href="mailto:support@hayaan.co"]')].find(x => x.textContent.includes("support@hayaan.co"));
  if (!a) return JSON.stringify({ found: false });
  a.scrollIntoView({ block: "center" });
  return JSON.stringify({ found: true, href: a.href, text: a.textContent.trim() });
})()`));
await sleep(1200);
await shot("/home/z/my-project/download/task59-footer-support-email.png");

// ---- 2. quote page: email line under the form ----
await send("Page.navigate", { url: `${BASE}/quote` });
await sleep(8000);
console.log("quote email line:", await ev(`(() => {
  const a = [...document.querySelectorAll('a[href="mailto:support@hayaan.co"]')].find(x => x.closest("p")?.textContent.includes("Write to us"));
  if (!a) return JSON.stringify({ found: false });
  a.scrollIntoView({ block: "center" });
  return JSON.stringify({ found: true, label: a.closest("p").textContent.trim().slice(0, 80) });
})()`));
await sleep(1200);
await shot("/home/z/my-project/download/task59-quote-email-line.png");
